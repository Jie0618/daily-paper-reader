...
## 摘要
从数据中重建非线性动力系统 (DS) 是科学和工程领域的一项基本挑战，但它本质上依赖于序列模型。序列模型的最新突破产生了能够沿序列长度 $T$ 并行化计算的算法，实现了对数级时间复杂度 $\mathcal{O}(\log T)$。由于经典随时间反向传播 (BPTT) 的线性运行时间复杂度 $\mathcal{O}(T)$ 限制了实际序列长度，这为动力系统重建 (DSR) 开辟了新的途径。本文研究了针对该任务的两类主要的时间并行算法，两者都利用并行关联扫描作为其核心计算原语。第一类包含具有线性但非自治动力学以及非线性读取的模型，例如现代状态空间模型 (SSMs)；而第二类由通用的非线性模型组成，可以使用 DEER 框架进行并行化。我们发现，第一类模型在训练时的线性递归特性带来了局限性，往往会阻碍对准确非线性动力学的学习。为了解决这个问题，我们将 DEER 与广义教师强制 (GTF) 相结合，这是更通用的非线性框架内的一种新变体，可确保在任意序列长度上稳定且有效地学习非线性动力学。利用 GTF-DEER，我们研究了在极长序列 ($T>10^4$) 上进行训练对 DSR 的益处。我们的结果表明，如果数据具有长的时间尺度，利用此类长轨迹可以显著提高 DSR 的性能。这项工作确立了 GTF-DEER 作为数据驱动发现的稳健工具，并强调了长序列学习在复杂动力系统建模中尚未被充分挖掘的巨大潜力。
