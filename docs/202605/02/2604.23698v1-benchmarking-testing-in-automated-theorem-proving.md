...
tldr: 针对大语言模型在形式化定理证明中语义正确性评估难的问题，本文提出了一种受代码生成测试启发的评估框架。该框架通过检查依赖于生成定理的后续定理是否能成功编译来验证其语义正确性。研究者基于5个Lean 4仓库构建了包含2206个问题的基准测试。实验发现，尽管现有模型编译成功率高，但在语义准确性上表现欠佳，揭示了当前定理生成能力的局限性。
selection_source: fresh_fetch
motivation: 现有的定理证明评估依赖词汇重叠或人工检查，难以准确衡量生成的定理在逻辑语义上的正确性。
method: 提出一种类似于集成测试的框架，通过验证所有依赖该定理的后续定理是否能成功编译来判定其语义正确性。
result: "构建了包含2206个Lean 4问题的基准，实验显示最强模型Claude-Sonnet-4.5的测试准确率仅为38.9%。"
conclusion: 实验结果表明当前大模型在生成语义正确的定理方面仍存在显著差距，传统的编译成功率指标不足以反映真实能力。
---

## 摘要
大语言模型（LLMs）的最新进展在形式化定理证明方面展现出潜力，但评估其语义正确性仍然具有挑战性。现有的评估方法依赖于间接指标，例如与人工标注证明的词汇重叠，或者是昂贵的人工检查。受代码生成领域从词汇比较转向基于测试的评估的启发，我们提出了 T，一个评估形式化定理语义正确性的框架：只有当所有依赖的后续定理都能成功编译时，生成的定理才被认为是正确的，这类似于集成测试。我们从 5 个真实的 Lean 4 仓库中构建了一个基准测试集，包含 2,206 个问题，平均每个问题配有 41 个后续定理，这些定理均是在无需人工干预的情况下自动提取的。实验表明，虽然最先进的模型实现了较高的编译成功率，但在我们的语义指标下表现明显较差。在同时提供自然语言证明和后续定理作为上下文的情况下，表现最好的模型 Claude-Sonnet-4.5 在全集上的测试准确率仅为 38.9%，揭示了当前定理生成能力中的关键差距。
