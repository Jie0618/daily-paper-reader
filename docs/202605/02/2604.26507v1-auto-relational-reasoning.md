...
tldr: "针对当前大型机器学习模型在推理能力上的局限性，本文提出了一种自动关系推理理论框架。该框架将对象关系推理与人工神经网络相结合，通过形式化分析构建了一种新型集成范式。在无需先验知识的情况下，该系统在智商测试问题上达到了98.03%的解决率，相当于132-144的智商水平。研究证明了通过结合机器学习的可扩展性与严谨的推理机制，可以有效提升系统的少样本或零样本问题解决能力。"
selection_source: fresh_fetch
motivation: 旨在解决大型机器学习模型在推理能力上的不足以及边际收益递减的问题。
method: 提出一种将自动化对象关系推理与人工神经网络协同集成的理论框架和形式化分析方法。
result: "该系统在无先验知识的情况下，以98.03%的成功率解决了IQ测试问题，达到人类前1%的智力水平。"
conclusion: 通过整合先验知识和扩大数据集，该系统可推广至更广泛的问题领域，并具备出色的零样本学习潜力。
---

## 摘要
背景与目标：在过去十年中，机器学习研究发展迅速，但大型模型正达到其软极限，表现出收益递减的趋势，且仍缺乏稳健的推理能力。通过将机器学习的可扩展性与严谨推理进行协同结合，可以突破这些限制。方法：在这项工作中，我们提出了一个理论框架，通过与人工神经网络集成的自动化方式进行对象关系推理。我们对该推理进行了形式化分析，并通过一个集成推理与机器学习的范式在实践中展示了该理论。结果：该范式是一个在没有任何先验知识的情况下解决智商（IQ）测试问题的系统。我们的系统达到了 98.03% 的解决率，相当于前 1% 的百分位，即 132-144 的 IQ 分数。这一结果仅受限于模型的小规模以及运行该模型的机器的处理能力。结论：通过在系统中集成先验知识并扩大数据集，该系统可以推广到解决大类问题。该系统的功能本质上有利于通过少样本（few-shot）或零样本（zero-shot）尝试来解决此类问题。
