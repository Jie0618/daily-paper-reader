...
tldr: 本研究探讨了AI在解决数学开放问题上的潜力，针对大语言模型在研究级证明中存在的背景污染、引用幻觉等七大失效模式，提出了开源多智能体系统QED。通过针对性架构设计，QED在应用分析和偏微分方程领域的五个开放问题中成功证明了三个，并获得专家验证，展示了AI生成原创性、非平凡数学证明的能力。
selection_source: fresh_fetch
motivation: 尽管大模型在基准测试中表现优异，但在生成原创且非平凡的数学研究级证明方面仍面临系统性挑战。
method: 提出了QED多智能体系统，通过特定的架构决策直接针对并解决大模型在证明过程中的七种主要失效模式。
result: 在专家提供的五个应用分析与偏微分方程开放问题中，QED成功生成了三个原创且非平凡的正确证明。
conclusion: 弥合基准测试与研究级证明之间差距的关键在于系统设计，QED证明了多智能体协作能有效提升AI解决复杂数学问题的能力。
---
