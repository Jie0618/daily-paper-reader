...
motivation: 现有的时间序列模型缺乏定性推理能力，而直接使用LLM处理数值数据会因分词器导致的模态差异而产生计算开销并损害数学关系。
method: 提出STRIDE框架，通过将轻量级LLM的推理状态动态投影为连续先验，并结合交叉熵与分位数损失进行联合优化，将语义推理注入TSFM的嵌入空间。
result: 在GIFT-Eval和TFRBench等基准测试中，STRIDE在数值预测和推理性能上均优于现有模型，并能有效增强Chronos-2等多种基础模型。
