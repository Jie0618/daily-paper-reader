...
method: 引入学习到的对角黎曼度量来扭曲输入空间，并开发了包括 LM-KAN 在内的多种变体，通过拉伸高变化区域和压缩平滑区域来优化分辨率分配。
result: GeoKAN 在处理科学机器学习中的尖锐、刚性和局部化问题时表现出色，能够将模型容量精准投放至最需要的区域。
conclusion: GeoKAN 通过引入几何归纳偏置，为物理信息学习和通用函数逼近提供了一种更具灵活性和效率的新架构。
---

## 摘要
我们引入了几何 Kolmogorov-Arnold 网络 (GeoKAN)，这是一类具有几何感知能力的 KAN 型模型。在该模型中，逼近过程是在学习到的、适应几何的坐标系中进行的，而非固定的欧几里得输入坐标系。GeoKAN 通过学习一个对角黎曼度量来实现这一点，该度量在基函数展开和特征混合之前对输入进行扭曲。学习到的度量通过局部长度缩放和体积畸变提供几何归纳偏置，并且在物理信息设置中，它还会影响模型所感知的微分结构。在此框架下，我们开发了三个主要变体，即 GeoKAN-NNMetric、GeoKAN-$γ$ 和 LM-KAN。对于 LM-KAN，我们进一步考虑了三种特定基函数的版本：LM-KAN-RBF、LM-KAN-Wav 和 LM-KAN-Fourier。这些变体使我们能够将几何感知 KAN 模型作为通用函数逼近器以及物理信息学习中的代理模型进行研究。通过拉伸变化剧烈的区域并压缩平滑区域，GeoKAN 以任务依赖的方式重新分配表示分辨率，使模型能够将容量分配到最需要的地方。因此，GeoKAN 非常适用于科学机器学习和微分方程问题中出现的尖锐、刚性、局部化以及强非均匀的情况。
