...
tags: ["query:sr-alg"]
score: 7.0
evidence: 发现城市系统的控制方程
tldr: 本研究针对城市地理系统缺乏确定性理论基础的问题，提出了一种统一的微分方程形式，将城市动态分解为空间相互作用和自身动态。研究开发了U-Discovery框架，利用大语言模型生成假设，并通过神经拟合方法从时空快照数据中识别控制方程。该方法在合成实验和真实人类活动数据中均表现出色，为发现城市系统演化规律提供了新途径。
selection_source: fresh_fetch
motivation: 城市系统动态演化缺乏明确的物理第一性原理指导，导致其背后的控制方程难以通过传统启发式方法发现。
method: 提出U-Discovery框架，整合了基于LLM的假设生成、神经拟合校准以及基于误差和复杂度的方程识别机制。
result: 实验证明U-Discovery能从模拟动态中精准识别唯一控制方程，并在真实世界的人类活动分析中表现优异。
conclusion: 该研究通过结合人工智能与微分方程建模，为自动化探索和理解复杂的城市空间相互作用机制提供了有力工具。
---
