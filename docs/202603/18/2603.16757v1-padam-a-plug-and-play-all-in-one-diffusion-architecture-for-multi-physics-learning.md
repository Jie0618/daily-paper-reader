...
## 摘要
在不同物理定律之间进行泛化仍然是科学人工智能面临的一个根本挑战。现有的深度学习求解器很大程度上局限于单方程设置，限制了在不同物理机制和推理任务之间的迁移。在这里，我们介绍了 pADAM，这是一个统一的生成框架，它在异构偏微分方程族之间学习共享的概率先验。通过学习系统状态以及（在适用时）物理参数的联合分布，pADAM 在单一架构内支持正向预测和反向推理，而无需重新训练。在从标量扩散到非线性 Navier-Stokes 方程的基准测试中，即使在稀疏观测下，pADAM 也能实现准确的推理。结合共形预测，它还提供了具有覆盖保证的可靠不确定性量化。此外，pADAM 仅通过两个稀疏快照即可执行概率模型选择，通过其学习到的生成表示来识别控制方程。这些结果突显了生成式多物理场建模在统一且具有不确定性意识的科学推理方面的潜力。
