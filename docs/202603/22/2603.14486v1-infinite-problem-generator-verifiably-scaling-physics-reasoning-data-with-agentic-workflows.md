...
tags: ["query:sr-alg"]
score: 6.0
evidence: 公式即代码范式，用于合成具有数学一致性的物理问题
tldr: 本研究针对物理推理领域高质量可验证数据稀缺的挑战，提出了“无限问题生成器”（IPG）框架。该框架采用“公式即代码”范式，将物理问题及其解法转化为可执行的Python程序，从而确保了数据的数学一致性与可验证性。通过对165个专家种子进行扩展，研究者构建了包含1335个问题的ClassicalMechanicsV1数据集。实验揭示了公式数量与代码长度间的强线性相关性，为衡量问题难度和实现可控的课程学习提供了精确指标。
selection_source: fresh_fetch
motivation: 针对物理等复杂推理领域中高质量、可验证数据稀缺以及传统文本增强方法易产生幻觉的问题。
method: 提出IPG框架，利用“公式即代码”范式将物理问题合成与Python程序执行相结合，确保解法的严谨性。
result: 成功构建了包含1335个经典力学问题的语料库，并证实了公式数量与验证代码长度之间存在极强的线性相关性。
conclusion: 该研究通过代码复杂度建立了精确的问题难度度量衡，为推理密集型领域的可控数据扩展和课程生成提供了有效工具。
---
