...
tags: ["query:sr-alg"]
score: 7.0
evidence: 通过结构化神经微分方程进行物理系统的数据驱动发现
...
## 摘要
许多工程物理过程表现出非线性但渐近稳定的动力学特性，这些特性收敛于由控制输入决定的有限平衡点集。从数据中识别此类系统具有挑战性：稳定的动力学提供的激励有限，且模型发现往往具有非唯一性。我们提出了一种最小结构化的神经常微分方程（NODE）架构，该架构通过学习形式为 $F(x,u) = f(x)\,(x - g(x,u))$ 的向量场，强制执行轨迹稳定性并为多稳态系统提供易于处理的参数化，其中逐元素满足 $f(x) < 0$ 以确保收缩性，而 $g(x,u)$ 决定了多吸引子的位置。在多个非线性基准测试中，所提出的结构在短时间跨度训练上表现高效，能够捕捉多个吸引盆，并通过隐式平衡映射 $g$ 实现高效的基于梯度的反馈控制。
