...
title_zh: UtilityMax Prompting：一种多目标大语言模型优化的形式化框架
authors: Ofir Marom
date: 2026-03-12
pdf: "https://arxiv.org/pdf/2603.11583v1"
tags: ["query:opt-lm"]
score: 8.0
evidence: 多目标大语言模型优化的正式数学框架
...
## 摘要
大语言模型（LLM）任务的成功在很大程度上取决于其提示词。大多数用例使用自然语言来指定提示词，但当需要同时满足多个目标时，自然语言本质上具有歧义性。在本文中，我们提出了 UtilityMax Prompting，这是一个使用形式化数学语言来指定任务的框架。我们将任务重构为一个影响图（influence diagram），其中 LLM 的回答是唯一的决策变量。我们在该图的条件概率分布上定义了一个效用函数，并指示 LLM 寻找能够使期望效用最大化的答案。这种方法约束了 LLM 对目标的每个组成部分进行显式推理，从而将其输出导向精确的优化目标，而非主观的自然语言解释。我们在 MovieLens 1M 数据集上，针对三种前沿模型（Claude Sonnet 4.6、GPT-5.4 和 Gemini 2.5 Pro）验证了我们的方法。实验结果表明，在多目标电影推荐任务中，该方法在准确率和归一化折损累计增益（NDCG）方面相较于自然语言基准取得了持续的提升。
