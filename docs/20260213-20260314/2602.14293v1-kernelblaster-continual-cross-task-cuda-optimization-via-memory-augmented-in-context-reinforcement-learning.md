...
tags: ["query:opt-lm"]
score: 7.0
evidence: 基于大语言模型的 CUDA 优化 GPU 编码智能体
...
## 摘要
在多代 GPU 架构上优化 CUDA 代码具有挑战性，因为实现峰值性能需要对日益复杂且特定于硬件的优化空间进行广泛探索。传统编译器受限于固定的启发式方法，而微调大语言模型（LLMs）的成本可能很高。然而，用于 CUDA 代码优化的智能体工作流（agentic workflows）聚合先前探索知识的能力有限，导致采样偏差和次优解。我们提出了 KernelBlaster，这是一个记忆增强式上下文强化学习（MAIC-RL）框架，旨在提升基于 LLM 的 GPU 编程智能体的 CUDA 优化搜索能力。KernelBlaster 通过将知识积累到可检索的持久化 CUDA 知识库中，使智能体能够从经验中学习，并在未来的任务中做出系统性的明智决策。我们提出了一种新型的基于性能分析引导和文本梯度的智能体流，用于 CUDA 生成和优化，以在各代 GPU 架构上实现高性能。KernelBlaster 引导 LLM 智能体系统地探索高潜力的优化策略，而不仅仅是简单的重写。与 PyTorch 基准相比，我们的方法在 KernelBench 的 Level 1、2 和 3 上分别实现了 1.43 倍、2.50 倍和 1.50 倍的几何平均加速。我们将 KernelBlaster 作为开源智能体框架发布，并附带测试工具集、验证组件和可复现的评估流水线。
