...
## 摘要
常微分方程 (ODE) 为建模广泛科学领域中出现的动力系统提供了一个强大的框架。然而，大多数现有的 ODE 方法都集中在单一系统上，未能充分解决从多个异构动力系统中学习共享模式的问题。在本文中，我们提出了一种用于建模异构 ODE 系统的创新分布鲁棒学习方法。具体而言，我们通过在由轨迹导数的凸组合形成的不确定性类别上最大化最坏情况下的奖励，构建了一个鲁棒动力系统。我们证明了所得估计量具有显式的加权平均表示，其中权重通过平衡多个数据源信息的二次优化获得。我们进一步开发了一种双层稳定化程序，以解决估计中潜在的不稳定性。我们为所提方法建立了严谨的理论保证，包括稳定权重的相合性、鲁棒轨迹估计的误差界以及逐点置信区间的渐近有效性。通过广泛的模拟实验和对颅内脑电图数据的分析，我们证明了与替代方案相比，所提方法显著提高了泛化性能。
