...
tags: ["query:math-ai"]
score: 6.0
evidence: 评估大语言模型在稳定和重写数值表达式方面的能力
tldr: 本研究系统评估了大语言模型（LLM）在科学软件数值稳定性方面的表现。通过检测数值不稳定性和重写表达式两个任务，对6种LLM在近2470个数值结构上进行了测试。结果显示，LLM在处理传统方法失效的复杂表达式时表现优异，但在处理控制流和高精度字面量时存在局限。该研究揭示了LLM作为传统数值优化工具补充的巨大潜力。
selection_source: fresh_fetch
motivation: 科学计算对浮点精度要求极高，但LLM在处理数值稳定性方面的可靠性尚未得到系统评估。
method: 通过检测不稳定输入和重写稳定表达式两个任务，评估了6种LLM在包含嵌套条件和高精度字面量的2470个数值结构上的表现。
result: "LLM在65.4%的表达式中优于基准方法，且能成功修复97.9%传统工具无法处理的数值不稳定问题。"
conclusion: LLM是传统数值稳定技术的有效补充，但在需要精确推理控制流语义和高精度数值量级的场景下仍面临挑战。
---

## 摘要
科学软件依赖高精度计算，然而有限的浮点表示可能会引入精度误差，并在安全关键领域中传播。尽管大语言模型（LLMs）在科学应用中的使用日益增多，但它们在处理浮点数值稳定性方面的可靠性尚未得到系统评估。本文通过两项数值稳定化任务评估了 LLM 在高精度数值计算上的推理能力：(1) 通过生成诱导误差的输入来检测数值表达式中的不稳定性（检测）；(2) 重写表达式以提高数值稳定性（稳定化）。利用流行的数值基准测试，我们在近 2,470 个数值结构上评估了六种 LLM，这些结构包括嵌套条件语句、高精度字面量和多变量算术。研究结果表明，在检测和稳定数值不稳定的计算方面，LLM 与最先进的传统方法同样有效。更值得注意的是，LLM 在基准方法失败的场景下表现更优：在基准方法未能提高精度的 17.4%（431 个）表达式中，LLM 成功稳定了其中的 422 个（97.9%），并在所有表达式的 65.4%（1,615 个）中实现了比基准方法更高的稳定性。然而，LLM 在处理控制流和高精度字面量时表现挣扎，往往倾向于移除此类结构而非推理其数值含义，而它们在纯符号表达式上的表现则显著更好。总之，这些发现表明 LLM 能够有效稳定经典技术无法处理的表达式，但在必须对精确数值量级和控制流语义进行精确推理时仍面临挑战，因为此类具体模式在训练过程中较少出现。
