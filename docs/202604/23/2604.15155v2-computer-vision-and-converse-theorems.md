...
date: 2026-04-16
pdf: "https://arxiv.org/pdf/2604.15155v2"
tags: ["query:math-ai"]
score: 6.0
evidence: 深度学习模型用于区分算术数据与随机矩阵
...
## 摘要
随机矩阵为一系列算术现象提供了一个成熟的统计模型。在本文中，我们研究了一维和二维卷积神经网络（CNN）在多大程度上能够区分源自导子在固定区间内的椭圆曲线算术数据，以及从相同 Sato-Tate 分布中提取的随机矩阵数据。受朗兰兹纲领（Langlands program）中逆定理的启发，我们将每条椭圆曲线及其扭曲（twists）表示为一个向量场，随后将该向量场编码为数字图像。我们观察到，与在不含扭曲数据的 Frobenius 迹向量上训练的一维 CNN 相比，在这些图像数据上训练的二维 CNN 能够更好地将导子族与随机矩阵数据区分开来。我们还观察到，相同的二维架构可以预测椭圆曲线的解析秩，并且它是通过（未扭曲的）Frobenius 迹进行分解来实现这一点的。
