...
date: 2026-04-13
pdf: "https://arxiv.org/pdf/2604.11284v1"
tags: ["query:math-ai"]
score: 7.0
evidence: 学习数学逻辑和算术的神经架构
tldr: "THEIA 是一种纯神经模块化架构，无需外部求解器即可学习 Kleene 三值逻辑。它通过专用领域引擎处理数学任务，在 K3 规则覆盖上达到 100%，且在长序列组合泛化中表现极佳（准确率 99.97%）。研究表明，模块化提供的结构化归纳偏置能诱导“延迟判定”机制，使其在处理不确定性逻辑推理时比 Transformer 更高效、更具泛化性。"
selection_source: fresh_fetch
motivation: 探索何种架构先验能使神经网络在不确定性环境下实现强组合泛化并完整掌握 Kleene 三值逻辑。
method: 提出模块化架构 THEIA，利用四个专用领域引擎处理不同数学域，并最终汇聚于逻辑模块进行推理。
result: "THEIA 实现了 K3 规则全覆盖，长序列泛化准确率达 99.97%，且训练速度比同类 Transformer 快 5.6 倍。"
conclusion: 模块化结构通过“延迟判定”机制实现了独特的组合策略，证明了结构化归纳偏置对复杂逻辑推理的重要性。
---

## 摘要
我们提出了 THEIA，这是一种模块化神经架构，能够在没有任何外部符号求解器的情况下端到端地学习完整的 Kleene 三值逻辑 (K3)，并研究了何种架构先验能够实现在不确定性下的组合泛化。THEIA 通过专用引擎处理四个数学领域（算术、排序、集合隶属关系、命题逻辑），这些引擎最终汇聚于一个逻辑模块。在包含 200 万个样本、输入空间约为 3.4x10^13 的数据集上进行训练，它在 5 个随机种子下均实现了 12/12 的 Kleene K3 规则覆盖，耗时 9.2 +/- 3.5 分钟（在相同设置下比参数量相当的 Transformer 快 5.6 倍）。一项模 3 顺序组合实验实现了从 5 步训练到 500 步评估的泛化，准确率达 99.97% +/- 0.02% —— 这一结果关键取决于结构化归纳偏置：将四引擎骨干网络替换为扁平的 MLP 后，无论容量如何（0.80M 和参数匹配的 2.75M 变体均失败），长度泛化在 50 步时便塌缩至随机水平；而采用相同协议训练的 pre-LN TF8LTuned Transformer 基准模型（3,582,147 个参数）在 500 步时达到了 99.24%（附录 D）。机械论探针揭示了模块化诱导了一种“延迟判定”：上游引擎编码特定领域的变量，而不确定最终的真值（探针准确率低于或等于 74% 的仅不确定性上限），判定仅在逻辑引擎边界处产生 —— 这一点通过激活修补得到了因果证实（在 986 对匹配样本上翻转率为 100%，在 n=5 个种子上重复实验；总计 100.0%）。Transformer 基准模型通过一种性质截然不同的表示轨迹（先收缩后扩张）达到了同等的正确性，这表明模块化架构和整体式架构实现了不同的组合策略。
