...
tldr: "针对大语言模型在不同难度数学问题上表现不一的问题，本文提出自适应多专家推理框架（AMR）。该框架通过路由系统预测问题难度与不确定性，动态调整采样策略，并结合多专家生成、神经验证器及聚类聚合技术。在GSM8K测试中，AMR仅使用原始数据便达到75.28%的准确率，优于多数使用合成数据的同类模型，证明了难度感知路由与不确定性引导聚合的有效性。"
selection_source: fresh_fetch
motivation: 解决大语言模型在处理不同难度数学问题时表现不稳定且缺乏针对性策略的问题。
method: 提出AMR框架，利用难度感知路由动态分配资源，结合多专家生成、多轮修正及基于聚类的答案聚合。
result: "在GSM8K数据集上，仅凭原始训练数据即实现75.28%的准确率，超越了许多依赖合成数据的7B参数模型。"
conclusion: 难度感知路由与不确定性驱动的聚合机制能显著提升数学推理模型的效率、鲁棒性及性能。
---

## 摘要
大语言模型（LLMs）在数学推理基准测试中表现出强劲的性能，但其在不同难度问题上的表现存在不一致的波动。本文介绍了自适应多专家推理（AMR），这是一个通过动态调整策略进行推理，从而关注问题复杂性的框架。一个专注于问题文本的敏捷路由系统能够预测问题的难度和不确定性，并引导可重构的采样机制来管理生成的广度。三个专门的专家生成候选回答，这些回答在多个修正和定稿阶段进行修改。神经验证器评估回答的正确性，而基于聚类的聚合技术则结合共识和回答质量来确定最终的候选答案。在 GSM8K 数据集上的评估显示，AMR 在仅使用原始训练数据的情况下达到了 75.28% 的准确率。这一结果优于大多数在合成数据上训练的同类 7B 模型。这表明，使用基于难度的路由和不确定性驱动的聚合模型，在提高数学推理模型的鲁棒性方面是高效且有效的。
