...
## 摘要
大语言模型在结构化逻辑推理方面表现出系统性局限：它们混淆了假设生成与验证，无法区分推测与已验证的知识，并允许薄弱的推理步骤在推理链中不受检查地传播。我们提出了一种符号推理支架，将皮尔斯（Peirce）的三位一体推理——溯因、演绎和归纳——转化为大语言模型辅助推理的显式协议。该框架通过五个代数不变量（Gamma Quintet）来强制执行逻辑一致性，其中最强的一个——“最弱环节”边界（Weakest Link bound）——确保推理链中的任何结论的可靠性都不会超过其支持程度最低的前提。这一原则在可能性逻辑中被独立确立为最弱环节消解，并在思维链推理中得到了实证验证，能够防止逻辑不一致在多步推理中累积。我们通过包含 100 个属性和 16 个模糊测试的基于属性的测试套件，在超过 10^5 个生成案例上验证了所有不变量，提供了一个经过验证的不变量参考实现，可作为未来推理基准测试的基础。
