...
title_zh: QED：一个用于生成开放数学问题证明的开源多智能体系统
...
tldr: 本研究探讨了AI在解决数学开放性问题上的潜力，针对大语言模型在生成原创证明时存在的上下文污染、幻觉及逻辑跳跃等七大失效模式，提出了开源多智能体系统QED。该系统通过针对性的架构设计克服了现有模型的局限。在应用分析和偏微分方程领域的五个开放问题测试中，QED成功解决了其中三个，证明结果经专家验证具有原创性且非平凡，为AI辅助数学研究提供了新路径。
selection_source: fresh_fetch
motivation: 尽管大语言模型在基准测试中表现优异，但在生成原创且非平凡的数学研究级证明方面仍面临系统性失效模式的挑战。
method: 提出了开源多智能体系统QED，其架构设计专门针对识别出的七种主要失效模式（如引用幻觉、证明计划不稳定等）进行优化。
result: 在五个由专家提供的应用分析与偏微分方程开放问题中，QED成功生成了三个正确且具有原创性的证明。
conclusion: 弥合基准测试与实际数学研究之间差距的关键在于系统设计，QED证明了多智能体协作能有效提升AI解决复杂数学问题的能力。
---

## 摘要
我们探讨了数学人工智能领域的一个核心问题：人工智能系统能否为开放的研究问题生成原创且非平凡的证明？尽管在基准测试中表现强劲，但生成真正新颖的证明对于大语言模型（LLMs）来说仍然是一个严峻的挑战。通过在前沿 LLMs 上进行研究级证明任务的系统性实验，我们识别出了阻碍可靠证明生成的七种失败模式，包括：上下文污染、引用幻觉、关键步骤的含糊其辞与证明精力的分配不当、不稳定的证明计划、缺乏重点的验证、问题修改以及单模型瓶颈。我们认为，基准测试的成功与研究级证明之间的差距主要源于系统设计，而这些失败模式正是其原因所在。我们提出了 QED，这是一个开源的多智能体证明系统，其每一项架构决策都直接针对特定的失败模式。在由领域专家提供的五个应用分析和偏微分方程（PDEs）开放问题上进行的评估显示，QED 为其中三个问题生成了正确的证明，且每个证明均被提供问题的专家验证为原创且非平凡。QED 已作为开源软件发布，网址为 https://github.com/proofQED/QED。
