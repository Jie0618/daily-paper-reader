...
tldr: 本研究针对组合优化问题，提出了一种名为 A2DEPT 的自动化算法设计框架。传统方法受限于固定模板，而 A2DEPT 将大语言模型视为系统级架构师，通过树状进化搜索和分层算子在广阔的程序空间内进行开放式合成。该方法引入了混合选择策略和反馈驱动的程序修复循环，确保了生成算法的可执行性与高效性。实验表明，A2DEPT 在多个基准测试中显著优于现有方法，有效降低了最优性差距。
selection_source: fresh_fetch
motivation: 现有的基于大语言模型的启发式算法设计通常依赖固定模板，限制了算法的系统级表达能力和搜索灵活性。
method: 提出基于进化程序树的 A2DEPT 框架，利用大语言模型进行开放式算法合成，并结合混合选择、分层算子及反馈驱动的修复机制。
result: "在标准和高约束基准测试中，A2DEPT 均优于现有基线，将平均归一化最优性差距相对最强基线降低了 9.8%。"
conclusion: A2DEPT 证明了通过进化程序树实现开放式、系统级算法合成的有效性，显著提升了自动化启发式算法设计的性能。
---
