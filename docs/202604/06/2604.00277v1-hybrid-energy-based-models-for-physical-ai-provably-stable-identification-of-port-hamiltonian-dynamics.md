...
motivation: 现有的能量模型在进行动力学系统辨识时，缺乏能够全局排除不稳定模式的正式稳定性保障，且面临表达力与稳定性之间的权衡。
method: 提出一种结合动态可见层与静态隐藏层的混合EBM架构，并利用Clarke导数和吸收不变性理论确保非光滑激活函数下的耗散稳定性。
result: 理论上证明了该架构在端口-哈密顿动力学下的稳定性，并在多阱和环形系统实验中展示了其优异的建模能力与安全保障。
conclusion: 该研究通过引入吸收不变性和混合架构，成功构建了兼具高表达力与严谨物理稳定性证明的系统辨识新框架。
---
