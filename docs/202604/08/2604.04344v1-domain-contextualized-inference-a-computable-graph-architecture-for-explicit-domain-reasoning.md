...
## 摘要
我们建立了一种与计算底层无关的推理架构，其中领域被作为显式的一等计算参数。这实现了领域范围的剪枝，将单次查询的搜索空间从 O(N) 降低到 O(N/K)；支持在符号、神经、向量及混合底层上进行与底层无关的执行；并产生了透明的推理链，其中每一步都携带其评估上下文。本文的贡献在于架构层面，而非逻辑层面。我们从五个维度对计算理论进行了形式化：一个五层架构；三种领域计算模式（包括链式索引、作为 Kleisli 组合的路径遍历，以及作为底层转换的向量引导计算）；一个包含 Query、Extend、Bridge 三种操作的底层无关接口；包含三类故障模式的可靠性条件 C1 至 C4；以及通过 PHQ-9 临床推理案例研究进行的验证。该计算理论（包括操作语义、复杂度界限、单子结构、底层转换以及边界条件）是本文的核心贡献。
