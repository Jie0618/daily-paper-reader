...
score: 6.0
evidence: 张量回归模型的秩选择
tldr: 本研究探讨了随机协变量设计下低秩张量回归的秩选择问题。通过推导CP和Tucker分解在Gaussian随机设计模型下的期望训练-测试差异（乐观度）的总体表达式，证明了乐观度在真实张量秩处达到最小。基于此，提出了一种面向预测的秩选择规则，并将其应用于图像回归和神经网络压缩，为深度学习中的模型选择提供了理论支持和实用工具。
selection_source: fresh_fetch
motivation: 旨在解决低秩张量回归模型中缺乏理论支撑的秩选择难题，并探索其在模型压缩中的应用。
method: 在高斯随机设计下，推导了CP和Tucker分解的期望训练-测试差异（乐观度）的渐近表达式。
result: 证明了乐观度在真实秩处取得最小值，并提出了一种与交叉验证一致的秩选择规则。
conclusion: 该方法不仅在图像回归任务中表现优异，还为神经网络的张量化压缩提供了有效的模型选择准则。
---
