...
tldr: 该研究提出了一种基于神经算子的数据驱动框架，旨在无需控制方程的情况下分析物理系统的稳定性和感受性。通过训练神经网络作为动力学仿真器，并利用自动微分提取雅可比矩阵，该方法能直接从观测数据中计算特征模态和预解模态。在混沌模型和高维流体动力学中的应用证明，该方法能有效识别非线性系统中的主导不稳定模式和输入输出结构，为复杂高维数据的动力学分析提供了强有力的工具。
selection_source: fresh_fetch
motivation: 传统的稳定性和感受性分析高度依赖已知的物理方程和线性化假设，限制了其在非线性或建模不全系统中的应用。
method: 通过训练神经网络作为动力学仿真器，并利用自动微分技术提取其雅可比矩阵，从而直接从观测数据中计算特征模态和预解模态。
result: 在混沌模型和高维流体流动实验中，该方法成功识别了强非线性状态下的主导不稳定模式和关键的输入输出动力学结构。
conclusion: 该研究提供了一种无需方程的通用动力学分析工具，能够从复杂高维数据中提取深层物理模式，对多个科学和工程领域具有重要价值。
---

## 摘要
理解复杂系统如何响应扰动（例如系统是否保持稳定，或者其最敏感的模式是什么）是科学和工程领域的一项基本挑战。传统的稳定性与感受性（预解算子）分析虽然功能强大，但依赖于已知方程和线性化，这限制了它们在非线性或建模不完善系统中的应用。在本文中，我们介绍了一种数据驱动框架，该框架仅通过观测数据即可自动识别稳定性属性和最优强迫响应，而无需控制方程。通过将神经网络训练为动力学仿真器，并利用自动微分提取其雅可比矩阵（Jacobian），我们可以直接从数据中计算特征模态和预解模态。我们在经典混沌模型和高维流体流动上演示了该方法，即使在强非线性状态下，也成功识别出了主导的不稳定模态和输入-输出结构。通过利用基于神经网络的仿真器，我们不仅可以轻松获得系统动力学的非线性表示，还能检索到以前难以解析的复杂动力学模式。这种无方程方法为分析复杂的高维数据集建立了一个广泛适用的工具，与气候科学、神经科学和流体工程等领域的重大挑战直接相关。
