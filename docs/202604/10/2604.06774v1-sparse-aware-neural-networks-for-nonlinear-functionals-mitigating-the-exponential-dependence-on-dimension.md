...
tags: ["query:sr-alg"]
score: 6.0
evidence: 利用稀疏感知神经网络学习非线性泛函和算子
tldr: 本研究针对算子学习中非线性泛函面临的维数灾难和可解释性差的问题，提出了一种稀疏感知神经网络框架。该框架结合卷积架构提取稀疏特征与深度全连接网络进行泛函逼近，利用通用离散化方法实现了从有限样本中的稳定恢复。研究证明，通过利用稀疏性，该方法在多种函数空间下显著提升了逼近速率并降低了样本需求，为缓解泛函学习中的指数级维度依赖提供了新的理论见解。
selection_source: fresh_fetch
motivation: 旨在解决深度神经网络在学习无限维函数空间算子时面临的维数灾难及可解释性不足的挑战。
method: 提出一种结合卷积架构提取稀疏特征与深度全连接网络逼近非线性泛函的混合框架，并采用通用离散化采样策略。
result: 在具有快速频率衰减和混合光滑性的函数空间中，实现了更优的逼近速率和更小的样本量需求。
conclusion: 稀疏性是缓解泛函学习中维数灾难的关键，为算子学习提供了更高效且具理论支撑的逼近方案。
---

## 摘要
深度神经网络已成为学习定义在无限维函数空间上的算子的强大工具。然而，现有理论经常遇到与维度相关以及可解释性有限的困难。本研究探讨了稀疏性如何帮助解决泛函学习中的这些挑战，而泛函学习是算子学习的核心组成部分。我们提出了一个框架，该框架利用卷积架构从有限数量的样本中提取稀疏特征，并结合深度全连接网络来有效地逼近非线性泛函。利用通用离散化方法，我们证明了稀疏逼近器能够从离散样本中实现稳定恢复。此外，确定性采样和随机采样方案对于我们的分析都是充分的。这些发现提高了在各种函数空间（包括具有快速频率衰减和混合光滑性的空间）中的逼近率并减少了样本量。它们还为稀疏性如何缓解泛函学习中的维度灾难提供了新的理论见解。
