...
## 摘要
大语言模型（LLMs）已展现出令人印象深刻的能力，但其处理推理密集型任务的内部机制仍有待深入探索。为了增进对模型内部处理机制的理解，我们通过检查任务执行期间的内部机制，研究了 LLMs 如何执行算术运算。利用早期解码技术，我们追踪了下一标记预测在各层之间是如何构建的。我们的实验表明，虽然模型能较早识别算术任务，但正确结果的生成仅发生在最后几层。值得注意的是，精通算术的模型在注意力模块和 MLP 模块之间表现出明确的分工，其中注意力模块负责传播输入信息，而 MLP 模块负责聚合信息。这种分工在能力较弱的模型中并不存在。此外，成功的模型似乎以功能化的方式处理更具挑战性的算术任务，这表明其具备超越事实召回的推理能力。
