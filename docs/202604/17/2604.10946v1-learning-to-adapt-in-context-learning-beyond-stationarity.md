...
## 摘要
Transformer 模型因其强大的经验性能，已成为广泛科学和工程领域的基础。其成功的核心能力之一是上下文学习（ICL）：当给定来自未知任务的短提示时，Transformer 可以在不进行任何参数更新的情况下执行逐 token 和下一 token 预测。最近的理论研究已开始揭示这一现象背后的机制，特别是在监督回归设置中。然而，这些分析主要假设任务分布是平稳的，这忽略了目标函数随时间变化的广泛现实场景。在这项工作中，我们通过提供非平稳回归问题下 ICL 的理论分析来填补这一空白。我们研究了门控线性注意力（GLA）机制如何适应不断变化的输入输出关系，并严格刻画了在这种动态设置下其相对于标准线性注意力的优势。为了对非平稳性建模，我们采用了一阶自回归过程，并证明 GLA 通过自适应地调节过去输入的影响——有效地实现了一种可学习的近期偏差——从而实现了更低的训练和测试误差。我们的理论发现得到了实验结果的进一步支持，验证了门控机制在非平稳 ICL 任务中的益处。
